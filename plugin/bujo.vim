...
autocmd BufNewFile todo.md call append(0, ['<!-- ╔╦╗╔═╗╔╦╗╔═╗ -->', '<!--  ║ ║ ║ ║║║ ║ -->', '<!--  ╩ ╚═╝═╩╝╚═╝ -->', '', '<!-- Directory: ' . split(expand('%:p:h:t'), '\v\n')[0] . ' -->', '<!-- Date: ' . strftime('%d %b %Y %X') . ' -->', ''])
" Update date on save
autocmd BufWritePost todo.md :keeppattern 6s/Date: \zs.*M/\=strftime('%d %b %Y %X')/ge
